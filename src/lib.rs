...
impl Lpsql {
	pub fn new(conf_fname: Option<&str>) -> Self {
		let conf_fname = conf_fname.unwrap_or("lpsql.toml");
		Self {
			conf: Conf::new(conf_fname) 
		}
	}
	pub fn _exec(&self, query: &str, params: Vec<QueryParam>) -> Result<Vec<String>, String> {
	...
	pub fn get_one(&self, query: &str, params: Vec<QueryParam>) -> Option<String> {
		match self._exec(query, params) {
			Err(e) => {
	...
	pub fn exec(&self, query: &str, params: Vec<QueryParam>) -> bool {
		match self._exec(query, params) {
			Err(e) => {
				println!("LPSQL ERR: {e}");
				return false
